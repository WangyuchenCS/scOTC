...
def evaluate_adata(eval_adata, cell_type, key_dic):
    ...
    """
    # PCA cluster results
    sc.tl.pca(eval_adata)  # 8001 × 6998
    sc.pl.pca(eval_adata, color=key_dic['condition_key'], frameon=False,
              title="PCA of " + cell_type + " by Condition", show=True, save="PCA of " + cell_type + " by Condition.png")
    ...
    print(cell_type, " common DEGs: ", common_nums)  #'CD4T':48
    ...
    # violin contrast
    gene = degs_ctrl[0]
    sc.pl.violin(eval_adata, keys=gene, groupby=key_dic['condition_key'], show=True)
    gene = degs_ctrl[1]
    sc.pl.violin(eval_adata, keys=gene, groupby=key_dic['condition_key'], show=True)
    gene = degs_ctrl[2]
    sc.pl.violin(eval_adata, keys=gene, groupby=key_dic['condition_key'], show=True)
...
def evaluate(data_name, eval_adata, key_dic, save_path=None, return_fig=False, eval_path=None):
    '''
    ...
    for cell_type in types:
        ...
        fig = sc.pl.pca(eval_adata, color=condition_key, frameon=False, show=True,
                        title="PCA of " + cell_type + " by Condition", return_fig=True,save="PCA of " + cell_type + " by Condition.png")
    #     fig = sc.pl.pca(
    #     eval_adata, color=condition_key,  frameon=False, show=True,
    #     title="PCA of " + cell_type + " by Condition",
    #     return_fig=True,
    #     save="PCA of " + cell_type + " by Condition.png",
    #     palette={"control": "#3b75af", "predicted": "#ef8636", "stimulated": "#519e3e"}
    # )
        ...
        for i in range(3):
            ...
            sc.pl.violin(eval_adata, keys=gene, groupby=condition_key, ax=ax, show=True,) 
